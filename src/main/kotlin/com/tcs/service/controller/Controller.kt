...
class Controller(private val service: Service,
    ...
    fun get(): ResponseEntity<ServiceResponse> {
        logger.info("Get All")
    ...
    @RequestMapping(value = ["/dapr"], method = [RequestMethod.GET], produces = [MediaType.APPLICATION_JSON_VALUE])
    fun getDapr(): MutableMap<String, String>? {
        logger.info("Get All dapr")
        var records = mutableListOf<Any>()
        //service.get().forEach { model -> records.add(model.data) }
        return  service.secretClient()
    }
